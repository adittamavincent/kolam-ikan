...
describe('useDraftSystem - Edge Cases (Localized V2)', () => {
    ...
    beforeEach(() => {
        ...
        mockSupabase.from = vi.fn((table: string) => {
            if (table === 'entries') {
                return { insert: vi.fn().mockReturnThis(), select: vi.fn().mockReturnThis(), single: vi.fn().mockResolvedValue({ data: { id: 'new-entry' }, error: null }), delete: vi.fn().mockReturnThis(), eq: vi.fn().mockReturnThis() };
            }
            if (table === 'sections') {
    ...
    it('should load initial drafts from local storage on mount', async () => {
        localStorage.setItem(
            ...
            JSON.stringify({
                sections: {
                    'inst-1': { personaId: 'persona-1', content: [{ type: 'paragraph', content: 'loaded' }], updatedAt: Date.now() },
                },
                updatedAt: Date.now(),
            })
        );
    ...
    it('should remove section from localStorage immediately when saveDraft(forceDelete) is called', async () => {
        ...
        const content = [{ id: 'b1', type: 'paragraph', content: [{ type: 'text', text: 'Will be removed' }] }] as PartialBlock[];
        ...
        let draft = JSON.parse(localStorage.getItem('kolam_draft_v2_stream-1') || '{}');
    ...
    it('should completely wipe localStorage when clearDraft is called', async () => {
        const { result } = renderHook(() => useDraftSystem({ streamId: 'stream-clear' }), { wrapper });
        await waitFor(() => expect(result.current.isLoading).toBe(false));

        const content = [{ id: 'b1', type: 'paragraph', content: 'Commit me' }] as PartialBlock[];

        act(() => {
            result.current.saveDraft('inst-commit', 'persona-a', content, 'Persona A');
        });
    ...
    it('should completely wipe localStorage when commitDraft is called', async () => {
       const { result } = renderHook(() => useDraftSystem({ streamId: 'stream-commit' }), { wrapper });
        await waitFor(() => expect(result.current.isLoading).toBe(false));
