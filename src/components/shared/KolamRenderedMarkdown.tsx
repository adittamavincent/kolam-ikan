...
function renderMarkdownBody(
  ...
  while (i < lines.length) {
    ...
    if (calloutMatch) {
      ...
      const title =
        calloutMatch[3].trim() || formatCalloutTitle(getCalloutType(rawType));
      ...
      nodes.push(
        ...
        >
          <summary>
            {renderInline(title, highlightTerm, `callout-${start}-summary`)}
          </summary>
    ...
    if (headingMatch) {
      ...
      activeSection = /^(citations|references)\s*$/i.test(
        headingMatch[2].trim(),
      )
        ? "citations"
        : null;
      nodes.push(
        renderHeading(level, headingMatch[2], `heading-${i}`, highlightTerm),
      );
      i += 1;
      continue;
    }
              ...
              <tr key={rowIndex}>
                {row.cells.map((cell, cellIndex) => (
                  <td
                    key={cellIndex}
                    style={{
                      textAlign: tableBlock.model.alignments[cellIndex],
                    }}
                  >
                    {renderInline(
                      cell.content,
    ...
    nodes.push(
      ...
      >
        {paragraphLines.flatMap((paragraphLine, lineIndex) => {
          ...
          return [
            <br key={`paragraph-${start}-br-${lineIndex}`} />,
            ...lineNodes,
          ];
        })}
      </PersonaSurface>,
