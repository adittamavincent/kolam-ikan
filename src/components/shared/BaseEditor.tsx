...
const livePreviewRevealEffect =
  StateEffect.define<LivePreviewRevealRange | null>({
    map: (value, changes) =>
      value
        ? {
...
export function resolveTableCellSourceOffset(
  cell: Pick<
    ...
    "content" | "paddingLeft" | "rawEnd" | "rawStart"
  >,
  offsetMap: number[],
...
export function getTableCellContentBounds(
  cell: Pick<
    MarkdownTableCellModel,
    "paddingLeft" | "rawContent" | "rawEnd" | "rawStart"
  >,
) {
  const rawCellWidth = Math.max(0, cell.rawEnd - cell.rawStart);
  ...
  const end = Math.max(
    start,
    Math.min(rawCellWidth - trailingWhitespace, rawCellWidth),
  );
...
export function resolveMeasuredTextPositionFromMeasuredCharacters(
  ...
  for (const character of lineCharacters) {
    if (clientX >= character.left && clientX <= character.right) {
      ...
      return {
        offset:
          clientX <= midpoint ? character.startOffset : character.endOffset,
        placement: "inside",
      };
    }
  }
...
function findTableCellBindingFromPoint(
  ...
  const bindingByElement = new Map(
    bindings.map((binding) => [binding.element, binding]),
  );
...
function applyTableHitFeedback(
  ...
  table
    .querySelectorAll(
      ".cm-kolam-table-hit-cell, .cm-kolam-table-hit-row, .cm-kolam-table-hit-col",
    )
    .forEach((node) => {
...
function getPreviewRangeFromShell(view: EditorView, shell: HTMLElement | null) {
  if (!shell) {
    return null;
  }
...
function buildTableControls(
  ...
  const beginDragInsert = (
    ...
    const applyIntent = (event: PointerEvent) => {
      ...
      if (axis === "column") {
        ...
        const anchor = getColumnBindingAtIndex(
          renderedRows,
          columnIndex,
          bindingByCell,
        );
      ...
      const anchor = getRowBindingAtIndex(
        renderedRows,
        rowIndex,
        bindingByCell,
      );
  ...
  const rebuildRails = () => {
    ...
    topRail.style.gridTemplateColumns = columnWidths
      .map((width) => `${width}px`)
      .join(" ");
    leftRail.style.gridTemplateRows = rowHeights
...
function attachTableSelection(
  ...
  table.addEventListener("pointerdown", (event) => {
    ...
    const visibleOffset = Math.max(
      0,
      Math.min(visiblePosition.offset, binding.offsetMap.length - 1),
    );
...
function attachHorizontalRuleSelection(
  ...
  hitbox.addEventListener("pointerdown", (event) => {
    ...
    const previewRange = getPreviewRangeForLineSpan(
      view,
      lineNumber,
      lineNumber,
    );
  ...
  finish() {
    ...
    this.entries
      .sort((left, right) => {
        ...
        const leftStartSide =
          (left.value as Decoration & { startSide?: number }).startSide ?? 0;
        const rightStartSide =
          (right.value as Decoration & { startSide?: number }).startSide ?? 0;
...
function findInternalAnchorLineFromHref(
  ...
  if (citationMatch) {
    ...
    for (let row = 0; row < lines.length; row += 1) {
      ...
      if (
        orderedMatch &&
        Number.parseInt(orderedMatch[1], 10) === citationNumber
      ) {
...
class CodeMirrorTableEditorAdapter {
  ...
  private nextSelection: { anchor: number; head: number } | null;
  ...
  getCursorPosition() {
    const head =
      this.nextSelection?.head ?? this.view.state.selection.main.head;
...
function buildLivePreviewDecorations(
    ...
    enter: (node) => {
      ...
      switch (node.name) {
        case "ATXHeading1":
          if (
            !revealSelection ||
            !intersectsSelection(state, node.from, node.to, true)
          ) {
        ...
        case "StrongEmphasis":
          if (
            !revealSelection ||
            !intersectsSelection(state, node.from, node.to)
          ) {
  ...
  findMarkdownTableBlocks(lines).forEach((block) => {
    ...
    block.model.rows.forEach((_, rowOffset) => {
      ...
      if (
        revealSelection &&
        intersectsSelection(state, line.from, line.to, true)
      ) {
  ...
  for (let lineNumber = 1; lineNumber <= state.doc.lines; lineNumber += 1) {
    ...
    if (
      lineNumber <= frontmatterEndLine ||
      isRowInsideFence(lines, lineNumber - 1)
    ) {
      continue;
    }
    ...
    if (orderedMatch) {
      ...
      for (let scan = lineNumber - 1; scan >= 1; scan -= 1) {
        ...
        maxMarkerLength = Math.max(maxMarkerLength, candidateMatch[2].length);
      }
...
function formatSelection(open: string, close = open): StateCommand {
...
export function computeMarkdownListContinuation(
  ...
  if (taskMatch) {
    ...
    if (shouldExitEmptyListItem(lineText, cursorOffset, prefixLength)) {
      return {
        exitList: true,
        from: 0,
        nextMarker: "",
...
export function continueMarkdownListCommand(): StateCommand {
  return ({ state, dispatch }) => {
    ...
    const continuation = computeMarkdownListContinuation(
      line.text,
      cursorOffset,
    );
...
export function shouldAutoCreateMarkdownTable(
  ...
  return (
    cursorOffset === lineText.length &&
    buildMarkdownTableFromHeaderLine(lineText) !== null
  );
}
...
function orderedListInputHandler() {
  return EditorView.inputHandler.of((view, from, to, text) => {
    ...
    if (
      text === " " &&
      shouldIgnoreOrderedListExtraSpace(line.text, cursorOffset)
    ) {
      return true;
    }
  ...
  },
  {
    ...
    run: (target: EditorView) =>
      (shouldTriggerTableEnter(target) &&
        runTableEditorCommand(target, (editor) =>
          editor.nextRow(MARKDOWN_TABLE_OPTIONS),
        )) ||
...
export default function BaseEditor({
        ...
        click: (event, view) => {
          ...
          if (href.startsWith("#")) {
            ...
            flashStartTimeoutRef.current = window.setTimeout(() => {
              if (
                clickToken !== activeClickTokenRef.current ||
                !viewRef.current
              )
                return;
  ...
  useEffect(() => {
    const nextMarkdown = normalizeOaiCitationsInMarkdown(
      typeof initialMarkdown === "string"
        ? initialMarkdown
        : blocksToStoredMarkdown(initialContent ?? []),
    );
