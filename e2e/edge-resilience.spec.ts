...
test.describe.serial("Empty States", () => {
  test("home dashboard loads for fresh account with no domains", async ({
    page,
  }) => {
    await goHome(page);
...
test.describe.serial("Edge Setup", () => {
  ...
  test("creates cabinet and stream for edge-case testing", async ({ page }) => {
    ...
    await expect(page.getByText(ctx.cabinetName)).toBeVisible({
      timeout: 5000,
    });
  ...
  test("creates persona for edge-case testing", async ({ page }) => {
    ...
    await expect(page.getByText(ctx.personaName)).toBeVisible({
      timeout: 5000,
    });
...
test.describe.serial("Domain Edge Cases", () => {
  ...
  test("creating domain with whitespace-only name is prevented", async ({
    page,
  }) => {
    await goHome(page);
...
test.describe.serial("Navigator Edge Cases", () => {
  test("cancelling stream creation with Escape removes the input", async ({
    page,
  }) => {
    restoreCtx();
    await goToDomain(page, ctx.domainId);
  ...
  test("right-click context menu shows rename and delete options", async ({
    page,
  }) => {
    restoreCtx();
    await goToDomain(page, ctx.domainId);
...
test.describe.serial("Rapid Interactions", () => {
  test("rapid domain switching does not crash", async ({ page }) => {
    ...
    await expect(page.locator("#headlessui-portal-root >> .fixed"))
