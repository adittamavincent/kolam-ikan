...
async function createDomainAndCapture(
  page: Page,
  name: string,
): Promise<string> {
  await goHome(page);
...
async function createStreamAndCapture(
  page: Page,
  name: string,
): Promise<string> {
...
async function ensurePersona(page: Page) {
  ...
  await expect(
    page.getByRole("heading", { name: "Manage Personas" }),
  ).toBeVisible();
...
test.describe.serial("Role: Mahasiswa Skripsi", () => {
  test("creates thesis domain with research structure", async ({ page }) => {
    ...
    roles.thesis.domainId = await createDomainAndCapture(
      page,
      roles.thesis.domainName,
    );

    await goToDomain(page, roles.thesis.domainId);
    await createCabinet(page, roles.thesis.cabinetName);
    roles.thesis.streamId = await createStreamAndCapture(
      page,
      roles.thesis.streamName,
    );
  ...
  test("uses canvas for thesis outline", async ({ page }) => {
    ...
    const editors = page.locator(
      '[class*="bn-editor"], [contenteditable="true"]',
    );
...
test.describe.serial("Role: Freelance Designer", () => {
  test("creates client domain with deliverables structure", async ({
    page,
  }) => {
    roles.designer.domainId = await createDomainAndCapture(
      page,
      roles.designer.domainName,
    );

    await goToDomain(page, roles.designer.domainId);
    await createCabinet(page, roles.designer.cabinetName);
    roles.designer.streamId = await createStreamAndCapture(
      page,
      roles.designer.streamName,
    );
  });
...
test.describe.serial("Role: Startup Founder", () => {
  test("creates product domain with feature streams", async ({ page }) => {
    roles.founder.domainId = await createDomainAndCapture(
      page,
      roles.founder.domainName,
    );

    await goToDomain(page, roles.founder.domainId);
    await createCabinet(page, roles.founder.cabinetName);
    roles.founder.streamId = await createStreamAndCapture(
      page,
      roles.founder.streamName,
    );
  });
...
test.describe.serial("Cross-Role: Multi-Domain Awareness", () => {
  ...
  test("switches to thesis domain by name and sees its structure", async ({
    page,
  }) => {
    await goToDomain(page, roles.thesis.domainId);
    await expect(page.getByText(roles.thesis.cabinetName)).toBeVisible({
    ...
    await expect(page.getByText(roles.thesis.streamName)).toBeVisible({
      timeout: 5000,
    });
  });

  test("switches to designer domain by name and sees its structure", async ({
    page,
  }) => {
    await goToDomain(page, roles.designer.domainId);
    await expect(page.getByText(roles.designer.cabinetName)).toBeVisible({
      timeout: 5000,
    });
    await expect(page.getByText(roles.designer.streamName)).toBeVisible({
      timeout: 5000,
    });
  });

  test("switches to founder domain by name and sees its structure", async ({
    page,
  }) => {
    await goToDomain(page, roles.founder.domainId);
    await expect(page.getByText(roles.founder.cabinetName)).toBeVisible({
      timeout: 5000,
    });
    await expect(page.getByText(roles.founder.streamName)).toBeVisible({
      timeout: 5000,
    });
  });
  ...
  test("stream data is isolated per domain", async ({ page }) => {
    ...
    await goToStream(page, roles.founder.domainId, roles.founder.streamId);
    await expect(page.getByText(/Google SSO/i)).toBeVisible({
      timeout: 10_000,
    });
