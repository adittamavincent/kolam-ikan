...
test.describe.serial("Workspace Setup", () => {
  ...
  test("creates a domain", async ({ page }) => {
    ...
    await expect(selectDomainByLabel(page, ctx.domainName)).toBeVisible({
      timeout: 5000,
    });
  });
  ...
  test("stream page shows entry creator with Commit Entry button", async ({
    page,
  }) => {
    await goToStream(page, ctx.domainId, ctx.streamId);
    await expect(
      page.getByRole("button", { name: "Commit Entry" }),
...
test.describe.serial("Entry Writing", () => {
  ...
  test("committed entry persists after page reload", async ({ page }) => {
    await goToStream(page, ctx.domainId, ctx.streamId);
    await expect(page.getByText(ctx.entryText)).toBeVisible({
      timeout: 10_000,
    });
  });
...
test.describe.serial("Canvas Writing", () => {
  ...
  test("canvas content persists after reload", async ({ page }) => {
    ...
    await goToStream(page, ctx.domainId, ctx.streamId);
    await expect(page.getByText(ctx.canvasText)).toBeVisible({
      timeout: 10_000,
    });
  });
});
