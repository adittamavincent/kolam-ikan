...
const createEntry = (
    ...
    sections: Array<{
        personaName: string;
        blocks: Array<{ type: string; text: string; props?: Record<string, unknown> }>;
    }>
): EntryWithSections => ({
    id,
    stream_id: 'stream-1',
...
describe('exportEntriesToMarkdown', () => {
    ...
    it('exports a single entry with one section correctly', () => {
        const entries = [
            createEntry('e1', '2026-01-15T10:00:00Z', [
                { personaName: 'Ibu Sari', blocks: [{ type: 'paragraph', text: 'Rantai makanan dimulai dari produsen.' }] },
            ]),
        ];
    ...
    it('exports multi-persona entry with both human and AI sections', () => {
        const entries = [
            createEntry('e1', '2026-01-15T10:00:00Z', [
                { personaName: 'Ibu Sari', blocks: [{ type: 'paragraph', text: 'Bagaimana cara menjelaskan fotosintesis?' }] },
                {
                    personaName: 'Dewi',
                    blocks: [{ type: 'paragraph', text: 'Gunakan diagram alur cahaya → klorofil → glukosa.' }],
                },
            ]),
        ];
    ...
    it('separates multiple entries with --- divider', () => {
        const entries = [
            createEntry('e1', '2026-01-10T08:00:00Z', [
                { personaName: 'Pak Hadi', blocks: [{ type: 'paragraph', text: 'Masa kecil di Surabaya.' }] },
            ]),
            createEntry('e2', '2026-01-11T09:00:00Z', [
                { personaName: 'Pak Hadi', blocks: [{ type: 'paragraph', text: 'Pertama kali ke Jakarta.' }] },
            ]),
            createEntry('e3', '2026-01-12T10:00:00Z', [
                { personaName: 'Pak Hadi', blocks: [{ type: 'paragraph', text: 'Mulai bekerja di koran.' }] },
            ]),
        ];
            ...
            createEntry('e1', '2026-01-15T10:00:00Z', [
                {
                    ...
                    blocks: [{ type: 'heading', text: 'Bab 1: Ekologi', props: { level: 2 } }],
                },
            ]),
        ];
            ...
            sections: [
                {
                    ...
                    content_json: 'not an array' as unknown as import('@/lib/types/database.types').Json,
                    search_text: '',
                    sort_order: 0,
                    created_at: '2026-01-15T10:00:00Z',
                    updated_at: '2026-01-15T10:00:00Z',
