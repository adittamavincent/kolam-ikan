...
async function ensureDocumentBucket() {
  ...
  const { data: existingBucket, error: getBucketError } = await admin.storage.getBucket(bucketId);
  if (!getBucketError && existingBucket) {
    return;
  }

  const { error: createBucketError } = await admin.storage.createBucket(bucketId, {
    public: false,
...
function getAppUrl(request: Request) {
  return process.env.DOCUMENT_IMPORT_APP_URL?.replace(/\/$/, '') ?? new URL(request.url).origin;
}

async function dispatchImportJob(request: Request, params: {
  jobId: string;
  documentId: string;
  streamId: string;
  title: string;
  ...
  const signedUrlResult = await admin.storage.from('document-files').createSignedUrl(params.storagePath, 60 * 60);
  if (signedUrlResult.error || !signedUrlResult.data?.signedUrl) {
    throw new Error(signedUrlResult.error?.message ?? 'Failed to create signed URL for document import');
  }
  ...
  if (!response.ok) {
    ...
    throw new Error(payload?.error ?? payload?.message ?? 'Document import worker rejected the job');
  }
}
...
export async function POST(request: Request) {
  ...
  if (!(file instanceof File) || typeof streamId !== 'string') {
    return NextResponse.json({ error: 'File and streamId are required' }, { status: 400 });
  }
  ...
  if (!input.success) {
    return NextResponse.json({ error: 'Invalid import parameters', details: input.error.flatten() }, { status: 400 });
  }
  ...
  const titleValue = input.data.title?.trim() || filename.replace(/\.pdf$/i, '');

  try {
    await ensureDocumentBucket();
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Failed to prepare document storage bucket';
  ...
  if (documentError && isMissingDocumentSchemaError(documentError.message)) {
    try {
      ...
      const message = error instanceof Error ? error.message : 'Failed to bootstrap document schema';
      return NextResponse.json({ error: message }, { status: 500 });
    }
    ...
    if (retriedDocumentInsert.error || !retriedDocumentInsert.data) {
      await admin.storage.from('document-files').remove([storagePath]);
      return NextResponse.json({ error: retriedDocumentInsert.error?.message ?? 'Failed to create document row' }, { status: 500 });
    }
    ...
    if (retriedJobInsert.error || !retriedJobInsert.data) {
      ...
      await admin.storage.from('document-files').remove([storagePath]);
      return NextResponse.json({ error: retriedJobInsert.error?.message ?? 'Failed to create import job row' }, { status: 500 });
    }
  ...
  if (!activeDocument && (documentError || !document)) {
    await admin.storage.from('document-files').remove([storagePath]);
    return NextResponse.json({ error: documentError?.message ?? 'Failed to create document row' }, { status: 500 });
  }
  ...
  if (!activeJob) {
    ...
    if (jobError || !job) {
      await admin.from('documents').delete().eq('id', documentId);
      await admin.storage.from('document-files').remove([storagePath]);
      return NextResponse.json({ error: jobError?.message ?? 'Failed to create import job row' }, { status: 500 });
    }
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'Failed to dispatch document import to worker';
  ...
  return NextResponse.json({ document: refreshedDocument ?? activeDocument, job: refreshedJob ?? activeJob }, { status: 201 });
}