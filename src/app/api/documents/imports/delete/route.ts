...
export async function POST(request: Request) {
  ...
  if (!body?.streamId || !body?.documentId) {
    return NextResponse.json({ error: 'streamId and documentId are required' }, { status: 400 });
  }
  ...
  const { data: document, error: documentError } = await admin
    .from('documents')
    .select('id, stream_id, import_status, storage_bucket, storage_path, deleted_at')
  ...
  if (document.import_status !== 'canceled') {
    return NextResponse.json({ error: 'Only canceled documents can be deleted' }, { status: 409 });
  }
