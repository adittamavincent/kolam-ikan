  ...
  parts.forEach((p) => {
    ...
    switch (k) {
      ...
      case "color":
        ...
        break;
      case "pad":
        style.padding = isNaN(Number(v)) ? v : `${v}px`;
        break;
      case "border":
        style.border = v;
        break;
  ...
  while ((m = regex.exec(text)) !== null) {
    ...
    if (m[1]) {
      ...
      nodes.push(
        React.createElement(
          "span",
          { style: parseAttrs(attrs), key: nodes.length },
          content,
        ),
      );
    } else {
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith(":::style")) {
      ...
      const children = blockLines
        ...
        .split("\n\n")
        .map((p, idx) =>
          React.createElement("p", { key: idx }, ...parseInlineFragments(p)),
        );
      out.push(
        React.createElement(
          "div",
          { className: "am-block", style, key: out.length },
          children,
        ),
      );
      continue;
    ...
    if (line.startsWith("```")) {
      ...
      out.push(
        React.createElement(
          "pre",
          { className: "am-code", style, key: out.length },
          React.createElement(
            "code",
            { "data-lang": lang },
            codeLines.join("\n"),
    ...
    if (line.trim().startsWith("|")) {
      ...
      let tableAttrs = "";
      if (i < lines.length && lines[i].trim().startsWith("{table:")) {
        tableAttrs = lines[i].trim().slice(1, -1);
        i++;
      }
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              null,
              header.map((h, idx) =>
                React.createElement("th", { key: idx }, h),
              ),
            ),
          ),
          React.createElement(
            ...
            body.map((r, ridx) =>
              React.createElement(
                "tr",
                { key: ridx },
                r.map((c, cidx) => React.createElement("td", { key: cidx }, c)),
              ),
            ),
          ),
        ),
