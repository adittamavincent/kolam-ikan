...
describe('Theme Accessibility Audit', () => {
  describe('Light Mode Contrast', () => {
    ...
    it('should have sufficient contrast for subtle text', () => {
      const ratio = getContrastRatio(themeColors.light.textSubtle, themeColors.light.background);
      expect(ratio).toBeGreaterThan(4.5); // AA Normal Text
    ...
    it('should have sufficient contrast for primary action buttons', () => {
      const ratio = getContrastRatio(themeColors.light.actionPrimaryText, themeColors.light.actionPrimaryBg);
    ...
    it('should have sufficient contrast for error messages', () => {
      const ratio = getContrastRatio(themeColors.light.statusErrorText, themeColors.light.statusErrorBg);
  ...
  describe('Dark Mode Contrast', () => {
    ...
    it('should have sufficient contrast for subtle text', () => {
      const ratio = getContrastRatio(themeColors.dark.textSubtle, themeColors.dark.background);
      expect(ratio).toBeGreaterThan(4.5);
      console.log(`Dark Mode Text Subtle Contrast: ${ratio.toFixed(2)}:1`);
    });

    it('should have sufficient contrast for primary action buttons', () => {
      const ratio = getContrastRatio(themeColors.dark.actionPrimaryText, themeColors.dark.actionPrimaryBg);
