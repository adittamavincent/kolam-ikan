...
function getStatusTone(status: string) {
  ...
  if (status === 'failed' || status === 'canceled') return 'bg-rose-500/15 text-rose-600';
...
export function DocumentImportModal({ isOpen, onClose, streamId, onSelectDocument }: DocumentImportModalProps) {
  ...
  const handleCancelDocument = async (documentId: string) => {
    ...
    try {
      ...
      setSubmitError(error instanceof Error ? error.message : 'Failed to cancel import.');
    }
  };
  ...
  const handleCancelAllPending = async () => {
    ...
    try {
      ...
      setSubmitError(error instanceof Error ? error.message : 'Failed to cancel pending imports.');
    }
  };
  ...
  const handleDeleteCanceledDocument = async (documentId: string) => {
    ...
    try {
      ...
      setSubmitError(error instanceof Error ? error.message : 'Failed to delete canceled document.');
    }
  };

  return (
    <Dialog open={isOpen} onClose={handleClose} className="relative z-50 transition duration-300 ease-out data-closed:opacity-0">
      ...
      <div className="fixed inset-0 flex items-center justify-center p-4">
        <DialogPanel className="mx-auto flex max-h-[90vh] w-full max-w-5xl flex-col gap-6 overflow-y-auto rounded-xl border border-border-default/50 bg-surface-default/95 p-8 shadow-[0_32px_64px_-12px_rgba(0,0,0,0.3)] backdrop-blur-xl transition duration-300 ease-out data-closed:translate-y-4 data-closed:scale-95 data-closed:opacity-0">
          <div className="flex items-start justify-between gap-4">
            <div>
              <DialogTitle className="text-2xl font-bold text-text-default">Import PDF</DialogTitle>
              <p className="mt-1.5 text-sm text-text-muted">
                Upload a source PDF into this stream and queue a Docling import job. Worker execution lands through the callback route.
              </p>
            </div>
            <button
              onClick={handleClose}
          ...
          <div className="grid gap-6 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
            <form onSubmit={handleSubmit} className="flex flex-col gap-4 rounded-2xl border border-border-default bg-surface-subtle/40 p-5">
              ...
              <label className="flex flex-col gap-2 text-sm text-text-default">
                ...
                <input
                  ...
                  placeholder={selectedFile ? selectedFile.name.replace(/\.pdf$/i, '') : 'Derived from filename'}
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <label className="flex flex-col gap-2 text-sm text-text-default">
                  ...
                  <select
                    value={flavor}
                    onChange={(event) => setFlavor(event.target.value as 'lattice' | 'stream')}
                    className="w-full rounded-lg border border-border-default bg-surface-default px-3 py-2 text-sm text-text-default outline-none transition-colors focus:border-action-primary-bg focus:ring-1 focus:ring-action-primary-bg"
                  >
                ...
                <div className="rounded-lg border border-border-default bg-surface-default px-3 py-2 text-sm text-text-muted">
                  <div className="font-medium text-text-default">Selected file</div>
                  <div className="mt-1 truncate">{selectedFile?.name ?? 'No file selected'}</div>
                  <div className="mt-1">{formatBytes(selectedFile?.size ?? null)}</div>
                </div>
              </div>
              ...
              <label className="flex items-center gap-3 rounded-lg border border-border-default bg-surface-default px-3 py-2 text-sm text-text-default">
                <input
                  ...
                  onChange={(event) => setEnableTableStructure(event.target.checked)}
              ...
              <label className="flex items-center gap-3 rounded-lg border border-border-default bg-surface-default px-3 py-2 text-sm text-text-default">
                <input
                  ...
                  onChange={(event) => setDebugDoclingTables(event.target.checked)}
                  className="h-4 w-4 rounded border-border-default"
                />
              ...
              <div className="flex items-center justify-between gap-3">
                <div className="text-xs text-text-muted">
                  Files are stored in the private document bucket and queued for worker pickup.
                </div>
                <button
                  type="submit"
            ...
            <div className="flex min-h-80 flex-col rounded-2xl border border-border-default bg-surface-subtle/40 p-5">
              ...
              <div className="mt-3 flex items-center justify-between gap-2 rounded-lg border border-border-default bg-surface-default px-3 py-2 text-xs text-text-muted">
                <span>{pendingDocuments.length} pending import{pendingDocuments.length === 1 ? '' : 's'}</span>
                <button
                  onClick={handleCancelAllPending}
                  disabled={pendingDocuments.length === 0 || cancelAllPendingImports.isPending}
                ...
                >
                  {cancelAllPendingImports.isPending ? 'Canceling...' : 'Cancel all pending'}
                </button>
              </div>
              ...
              <div className="mt-4 flex-1 space-y-3 overflow-y-auto pr-1">
                ...
                {documents.map((document) => (
                  <div key={document.id} className="rounded-xl border border-border-default bg-surface-default px-4 py-3">
                    {(() => {
                      const latestJob = document.latestJob;
                      const status = latestJob?.status ?? document.import_status;
                      const progressPercent = latestJob?.progress_percent ?? (status === 'completed' ? 100 : 0);
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <div className="truncate text-sm font-semibold text-text-default">{document.title}</div>
                        <div className="truncate text-xs text-text-muted">{document.original_filename}</div>
                      </div>
                      <span className={`rounded-full px-2 py-0.5 text-[11px] font-semibold uppercase tracking-wide ${getStatusTone(status)}`}>
                        {status}
                      </span>
                    </div>

                    <div className="mt-3 grid gap-1 text-xs text-text-muted">
                      <div>Created: {formatDate(document.created_at)}</div>
                      <div>Size: {formatBytes(document.file_size_bytes)}</div>
                      <div>Latest job: {status}</div>
                      {progressMessage && <div>Progress: {progressMessage}</div>}
                      {eta && <div>ETA: {eta}</div>}
                      {latestJob?.error_message && <div className="text-rose-600">Error: {latestJob.error_message}</div>}
                    </div>
                    ...
                    {(status === 'queued' || status === 'processing') && (
                      <div className="mt-3 flex justify-end">
                        <button
                          ...
                          disabled={cancelImport.isPending || cancelAllPendingImports.isPending || deleteCanceledDocument.isPending}
                    ...
                    {status === 'canceled' && (
                      <div className="mt-3 flex justify-end">
                        <button
                          onClick={() => handleDeleteCanceledDocument(document.id)}
                          disabled={deleteCanceledDocument.isPending || cancelImport.isPending || cancelAllPendingImports.isPending}
                        ...
                        >
                          {deleteCanceledDocument.isPending ? 'Deleting...' : 'Delete'}
                        </button>
                      </div>
                    )}

                    {(status === 'queued' || status === 'processing' || progressPercent > 0) && (
                      <div className="mt-3">
                        <div className="mb-1 flex items-center justify-between text-[11px] text-text-muted">
                          <span>{progressPercent}%</span>
                          {status === 'processing' && <span>{progressMessage ?? 'Processing document'}</span>}
                        </div>
