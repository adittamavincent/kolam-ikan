...
export async function POST(request: Request) {
  ...
  if (!parsed.success) {
    return NextResponse.json({ error: 'Invalid callback payload', details: parsed.error.flatten() }, { status: 400 });
  }
  ...
  if (currentJob.status === 'canceled' && payload.status !== 'canceled') {
    return NextResponse.json({ ok: true, ignored: true, reason: 'job already canceled' });
  }

  const now = new Date().toISOString();
  const jobPatch = {
    status: payload.status,
    progress_percent: payload.progressPercent ?? (payload.status === 'completed' ? 100 : undefined),
    ...
    completed_at: payload.status === 'completed' || payload.status === 'failed' || payload.status === 'canceled' ? now : null,
  };
