...
create or replace trigger update_document_import_jobs_updated_at
...
values ('document-files', 'document-files', false, 52428800, array['application/pdf'])
...
create policy "Users can view stream documents bucket objects"
on storage.objects
for select
to authenticated
using (
...
create policy "Users can upload stream documents bucket objects"
on storage.objects
for insert
to authenticated
with check (
...
create policy "Users can update stream documents bucket objects"
on storage.objects
for update
to authenticated
using (
...
create policy "Users can delete stream documents bucket objects"
on storage.objects
for delete
to authenticated
using (
