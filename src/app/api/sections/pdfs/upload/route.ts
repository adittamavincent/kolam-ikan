...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ error: 'Failed to parse PDF metadata' }, { status: 400 });
  }
  ...
  if (insertError || !inserted) {
    ...
    return NextResponse.json({ error: insertError?.message ?? 'Failed to create PDF document row' }, { status: 500 });
  }

  const signed = await admin.storage.from('document-files').createSignedUrl(storagePath, 60 * 30);
